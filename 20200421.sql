 ...
 함수 : 로직을 모듈화 한 코드
 ==> 실제 사용(호출)하는 곳과 함수가 구현되어 있는 부분을 분리
 ==> 유지보수의 편이성을 도모

 함수를 사용하지 않을 경구
 ==> 호출하는 부분에 함수 코드를 직접 기술해야 하므로, 코드가 길어진다
 ==> 가독성이 나빠진다
 ...
 SUBSTR(문자열, 시작 인덱스, 종료 인덱스) : 문자열의 시작 인덱스부터 종료 인덱스까지의 부분 문자열
 시작 인덱스는 1부터 (*java의 경우는 0부터이고 종료 인덱스 -1까지의 부분 문자열)
 ...
 TO_DATE(문자열, 포멧) : 문자열을 포멧에 맞게 해석하여 날짜 타입으로 형변환
 TO_CHAR(날짜, 포멧) : 날짜타입을 포멧에 맞게 문자열로 변환
